document.addEventListener("DOMContentLoaded", () => {
  ...
  // Build share buttons HTML for an activity card
  function createShareButtonsHtml(activityName, activityDescription) {
    const shareUrl = createShareUrl(activityName);
    const shareText = encodeURIComponent(
      `Check out ${activityName} at ${SCHOOL_NAME}! ${activityDescription}`
    );
    ...
    return `
      <div class="share-buttons">
        ...
        <button class="share-btn share-btn-copy" data-share-url="${shareUrl}" title="Copy link">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"></path></svg>
        </button>
      </div>
    `;
  }
  ...
  function renderActivityCard(name, details) {
    ...
    // Highlight card if URL contains matching ?activity= param
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get("activity") === name) {
      activityCard.classList.add("activity-card-highlighted");
      // Scroll to this card after rendering
      setTimeout(() => activityCard.scrollIntoView({ behavior: "smooth", block: "center" }), 300);
    }
    ...
    activityCard.innerHTML = `
      ...
      ${createShareButtonsHtml(name, details.description)}
    `;
